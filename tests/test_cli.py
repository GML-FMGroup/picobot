"""Tests for picobot CLI behavior."""
...
class CLITests(unittest.TestCase):
    ...
    def test_script_entrypoint_accepts_m(self) -> None:
        ...
        script_path = project_root / "picobot-cli"
    ...
    def test_cmd_message_collects_final_text(self) -> None:
        ...
        fake_agent = pytypes.SimpleNamespace(name="picobot")
        fake_agent_module = pytypes.SimpleNamespace(root_agent=fake_agent)

        with patch.dict("sys.modules", {"picobot.agent": fake_agent_module}):
            with patch("picobot.cli.InMemorySessionService", _FakeSessionService):
                with patch("picobot.cli.Runner", _FakeRunner):
