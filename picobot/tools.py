"""Core tools for picobot (except spawn)."""
...
def _workspace() -> Path:
    workspace_env = os.getenv("PICOBOT_WORKSPACE")
...
def web_fetch(url: str, max_chars: int = 50000) -> str:
    ...
    req = Request(url, headers={"User-Agent": "picobot/0.1"}, method="GET")
    try:
...
def _cron_store_path() -> Path:
    return _workspace() / ".picobot" / "cron_jobs.json"
...
def _debug_enabled() -> bool:
    return os.getenv("PICOBOT_DEBUG", "").strip().lower() in {"1", "true", "yes", "on"}
